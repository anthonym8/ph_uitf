...
One known advantage of UITFs is the low initial capital required to start investing. Typically advertised starting capital is P10,000. Looking at our dataset, the median starting capital required is indeed P10,000.  
...
| Category:         | Invested in:                      | Risk Flavor:      |
|:------------------|:----------------------------------|:------------------|
...
| Money Market Fund | Short-term debt instruments       | Conservative      |

<br />
The risk flavors for each fund category above is not fixed. These are the typical labels used by banks when marketing their UITF products. The graph below shows how each fund category is labelled by risk flavor according to banks.

```{r warning=FALSE, message=FALSE}
plot <- UITF_matrix %>% 
    select(Category = `Fund Classification`, Risk = `Risk Classification`) %>% 
    mutate(Category = factor(Category, levels = c("Equity Funds",
                                                  "Balanced Funds",
    ...
    mutate(Total = n()) %>% 
    ungroup() %>% 
    group_by(Category, Risk) %>% 
    mutate(Count = n(), Ratio = n()/Total) %>% 
    ungroup() %>% 
    distinct() %>% 
    mutate(tooltip = paste(Category,
                           paste("Risk:", Risk),
                           paste(Count, "out of", Total, "products"),
                           sep = "\n")) %>% 
    arrange(as.integer(Category), Risk) %>% 
    ggplot(aes(x = reorder(Category, desc(Category)), y = Ratio, fill = Risk)) +
    geom_bar_interactive(aes(data_id = "any", tooltip = tooltip),
    ...
    labs(title = "Risk Classifications by Fund Category",
         x = "Fund Category",
         y = "Ratio") +
    scale_fill_brewer(palette = "Set1") +
