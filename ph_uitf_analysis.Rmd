...
library(DT)         # for interactive output tables
...
After reading in the file again with specified column data types, we transform the date columns from strings to date using functions from the *lubridate* package.
...
#### **Select PSEi-based UITFs**
note to self: bloomberg scraper https://www.bloomberg.com/markets/chart/data/1Y/BPIEQUI:PM

```{r warning=FALSE}
uitf_df %>% 
    # filter(Bank %in% c("BDO Unibank, Inc.", "BPI Asset Management and Trust Corporation")) %>%
...
search_bloomberg_symbol <- function(search_string) {
    # search_string <- "www.bloomberg.com/quote BPI Equity Value Fund"
    html <- getForm("https://www.google.com/search", hl="tl",
                lr="", q=search_string, btnG="Search") %>% 
    htmlTreeParse()
    ...
    if(symbol == "") {
        symbol <- html$children$html[2]$body[2]$table[4]$tbody[2]$tr[2]$td[1]$div[2]$div[2]$div[1]$div[1]$ol[1]$div[1]$h3[1]$a[1]$b[1]$text %>% 
    toString() %>% 
    str_split(pattern = " ") %>% 
    unlist() %>% 
    `[`(1)
