...
plot_data <- UITF_matrix %>% 
    ...
    mutate(tooltip = paste(Bank,
                           paste(n(), "out of", Total , "products"),
                           sep = "\n")) %>% 
    ungroup() %>% 
    mutate(Currency = factor(Currency, levels = c("USD", "PHP")))

plot <- plot_data %>% 
    ggplot(aes(x = reorder(Bank, desc(Total)), fill = Currency)) +
    geom_bar_interactive(aes(tooltip = paste(..count.., "products"))) +
...
A complete list of UITFs can be browsed using the interactive table below.
```{r message=FALSE, warning=FALSE}

UITF_matrix %>% 
    select(Bank, Name, Currency) %>% 
...
plot <- UITF_matrix %>% 
    ...
    mutate(tooltip = paste(paste0("Bank: ", Bank),
                           ...
                           sep="\n")) %>% 
    ggplot(aes(x=reorder(Bank, desc(Total)), y = Ratio, 
