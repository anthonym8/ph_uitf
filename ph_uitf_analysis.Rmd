...
<br />
We can plot a quick line graph of our price data. This graph does not provide much insight and majority of UITFs are compressed in the lower region of the vertical axis. However, this shows that we have our data in a nice format we can work with later on.
```{r warning=FALSE, message=FALSE}

plot <- price_data %>% 
    ggplot(aes(x=Date, y=NAVPU, col=UITF)) +
    geom_line_interactive(aes(tooltip=UITF)) +
...
# dot plot of UITF introductions
plot <- uitf_df %>% 
    transmute(Year = year(`Inception Date`), Bank = Bank) %>% 
    ggplot(aes(x = Year, fill = Bank, col = Bank)) +
    geom_dotplot(method = "histodot", binwidth = 1, dotsize = 0.7, stackdir = "center", stackgroups = TRUE) +
    coord_cartesian(xlim = c(1994, 2018)) +
    scale_x_continuous(breaks = seq(1990,2020, by = 2)) +
    scale_y_continuous(breaks = NULL) +
    labs(x = "Year", y = "Count", title = "Timeline of UITF Inceptions") +
...
price_data %>% 
    ...
    geom_boxplot(col = "black", fill = "white", width = 0.07, alpha = 0.5) +
