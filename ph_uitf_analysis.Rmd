...
plot <- uitf_df %>% 
    ...
    geom_bar_interactive(aes(tooltip = ..count..), fill = "darkgreen", width = 0.95) +
...
plot <- uitf_df %>% 
    ...
    geom_dotplot(method = "histodot", binwidth = 1, dotsize = 0.55, stackdir = "center", stackgroups = TRUE) +
...
plot <- uitf_df %>% 
    ...
    geom_bar_interactive(aes(tooltip = ..count..), position = "fill", width = 0.95) +
    theme(plot.title = element_text(hjust = 0.5, size = 12),
          axis.title.y = element_text(size = 9),
          axis.text.y = element_text(size = 8),
...
ggplot(filter(scaled_prices, Classification=="Money - Market Funds"), aes(x=Date, y=Return, group=UITF, col=`Bank`)) +
    geom_line_interactive(aes(tooltip=UITF), alpha=0.8) +
