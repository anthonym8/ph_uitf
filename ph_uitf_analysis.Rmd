...
This section describes the data gathering and processing steps. The actual analysis starts in the subsequent section. Below are the three data sets necessary for this analysis, all of which are scraped from the internet. The data gathering pipeline for each are described after  

|     | Dataset                   | Description                                   |
|:---:|:--------------------------|:----------------------------------------------|
|  1  | UITF Matrix               | Complete list of all available UITFs in the                                          Philippines + other important information for                                        each UITF |
|  2  | Bloomberg ticker symbols  | Ticker symbols used in Bloomberg.com. This is for                                     extracting daily price data. |
|  3  | Daily price data          | Daily price data for the the past year for each                                      UITF. This is for comparing performance between                                      funds. |
...
# clean fund names
for(i in seq_along(uitf_df$Bank)) {
    message(i)
    uitf_df$`Fund Name`[i] <- uitf_df$`Fund Name`[i] %>% 
        str_split(" ") %>% 
        unlist() %>% 
        map_chr(
            function(x){
                ifelse(
                    str_detect(string = uitf_df$Bank[i], pattern = x),
...
paged_table(uitf_df[1:9, c(1,2,3,5)])
...
#### Bloomberg Symbol Lookup

Our second dataset is a mapping of Bloomberg ticker symbols to our list of UITFs. This is a requirement for scraping daily price data later. For this, we're using Bloomberg's Symbol Lookup because it can be queried easily using urls in this format: `www.bloomberg.com/markets/symbolsearch?query=` + `<search string>`
...
This function queries *Google Search* and returns the ticker symbol we need. All we need to do is pass it a string which will be used for our google search. The string follows this format: `www.bloomberg.com/quote` + `<bank name>` + `<UITF name>`. Expand the code chunk below to see the function code.
...
search_bloomberg_symbol <- function(search_string) {
    ...
    html <- getForm("http://www.google.com.ph/search", hl="en", lr="", q=search_string, btnG="Search") %>%
        htmlTreeParse()
...
#### Extracting data from Bloomberg.com
